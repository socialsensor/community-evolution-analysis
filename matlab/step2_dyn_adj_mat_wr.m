...
function step2_dyn_adj_mat_wr(folder_path,timeSeg) %%Comment this line if you need the script
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mkdir(folder_path,['\data\txts\adjMats\timeSeg_',num2str(timeSeg)]);
mkdir(folder_path,['\data\mats\adjMats\timeSeg_',num2str(timeSeg)]);
...
load([folder_path,'\data\mats\mentionLim_',num2str(timeSeg),'.mat'],'mentionLimit');
load([folder_path,'\data\mats\firstderiv_',num2str(timeSeg),'.mat'],'firstderiv');
firstderiv(end)=0;
sesStart=1;
bin=1;
uniqueUsers={};
for k=1:length(mentionLimit)
    if firstderiv(k)<0 && firstderiv(k+1)>=0
        ...
        authored=tempUsers(authIdx,1);
        mentioned=tempUsers(mentIdx,1);
        %save csvs of all the dynamic adjacency matrices in gephi-ready
        %form (the file name numbers correspond to the various POIs)
        fid = fopen([folder_path,'\data\txts\adjMats\timeSeg_',num2str(timeSeg),'\adjMat_',num2str(k),'.csv'],'w');
        ...
        save([folder_path,'\data\mats\adjMats\timeSeg_',num2str(timeSeg),'\adjMat_',num2str(bin),'.mat'],'adjMat');
        ...
        save([folder_path,'\data\mats\adjMats\timeSeg_',num2str(timeSeg),'\tempUsers_',num2str(bin),'.mat'],'tempUsers');
        sesStart=sesEnd+1;
        bin=bin+1;
    end
end
%save a mat of a cell array containing all unique users' names
save([folder_path,'\data\mats\adjMats\timeSeg_',num2str(timeSeg),'\uniqueUsers.mat'],'uniqueUsers');
