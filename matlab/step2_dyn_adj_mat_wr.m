...
mkdir(folder_name,['\txts\adjMats\timeSeg_',num2str(timeSeg)]);
mkdir(folder_name,['\mats\adjMats\timeSeg_',num2str(timeSeg)]);
fid = fopen([folder_name,'\txts\authors_mentions_time.txt']);
C = textscan(fid,'%s %s %*[^\n]', 'CollectOutput');%*s %*s %u8 %s
fclose(fid);
authors=C{1}; mentions=C{2};%complete authors & mentions
clear C fid lngth templA txtStartFile txtEndFile place
load([folder_name,'\mats\time\mentionLim_',num2str(timeSeg),'.mat'],'mentionLimit');
load([folder_name,'\mats\time\firstderiv_',num2str(timeSeg),'.mat'],'firstderiv');
firstderiv(end)=0;
sesStart=1;
bin=1;
uniqueUsers={};
...
%         fprintf(fid,'%s\n','Source,Target,Weight');
        fid = fopen([folder_name,'\txts\adjMats\dynamic\timeSeg_',num2str(timeSeg),'\test',num2str(k),'.csv'],'w');
...
%             fprintf(fid,'%s,%s,%d\r\n', authored{row,:},mentioned{row,:},weight(row,:));
        ...
        save([folder_name,'\mats\adjMats\dynamic\timeSeg_',num2str(timeSeg),'\adjMat_',num2str(bin),'.mat'],'adjMat');
        ...
        save([folder_name,'\mats\adjMats\dynamic\timeSeg_',num2str(timeSeg),'\tempUsers_',num2str(bin),'.mat'],'tempUsers');
        sesStart=sesEnd+1;
        bin=bin+1;
    end
end
%save a mat of a cell array containing all unique users' names
save([folder_name,'\mats\adjMats\dynamic\timeSeg_',num2str(timeSeg),'\uniqueUsers.mat'],'uniqueUsers');
