...
CommDir=dir([folder_name,'\mats\adjMats\dynamic\timeSeg_',num2str(timeSeg),'\adjMat*.mat']);
lDir=length(CommDir);
...
for k=1:lDir%%%%%%%%%%%%%%%%%%%%%%!!!!!!!!!!!!!!!!!!
    load([folder_name,'\mats\adjMats\dynamic\timeSeg_',num2str(timeSeg),'\adjMat_',num2str(k),'.mat']);
...
save([folder_name,'\mats\timeSeg_',num2str(timeSeg),'\commStructure.mat'],'commStructure');
matlabpool close
toc
for k=1:lDir%%%%%%%%%%%%%%%%%%%%%%!!!!!!!!!!!!!!!!!!
    ...
    load([folder_name,'\mats\adjMats\dynamic\timeSeg_',num2str(timeSeg),'\tempUsers_',num2str(k),'.mat']);
    ...
    save([folder_name,'\mats\timeSeg_',num2str(timeSeg),'\strComms',num2str(k),'.mat'],'strComms');
    %save corresponding community number to each user
    save([folder_name,'\mats\timeSeg_',num2str(timeSeg),'\tempUsersCommNums',num2str(k),'.mat'],'tempUsersCommNums');
end
%save a modularity vector for each timeslot
save([folder_name,'\mats\timeSeg_',num2str(timeSeg),'\modularity.mat'],'mymodularity');
% save community sizes
save([folder_name,'\mats\timeSeg_',num2str(timeSeg),'\commSizes.mat'],'commSizes');
