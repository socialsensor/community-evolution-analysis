...
if dataextractStage==1:#If the basic data(authors, mentions, time) has NOT been created
    ...
    if not os.path.exists(dataset_path + "/data/nonadaptive/tmp/"):
    ...
    if not os.path.exists(dataset_path + '/data/nonadaptive/tmp/dataComm_'+timeTitle+'.pck'):
        data = pickle.load(open(dataset_path + "/data/nonadaptive/tmp/data.pck", 'rb'))
    else:
        data = pickle.load(open(dataset_path + '/data/nonadaptive/tmp/dataComm_'+timeTitle+'.pck', "rb"))
    data.dataset_path=dataset_path
    dataEvol=data.evol_detect(prevTimeslots, xLablNum)
    del(data)
    dataEvolPck = open(dataset_path + "/data/nonadaptive/tmp/dataEvol_prev"+str(prevTimeslots)+timeTitle+".pck", "wb")
    pickle.dump(dataEvol, dataEvolPck, protocol = 2)
    dataEvolPck.close()
    elapsed = time.time() - t
    print('Stage 2: %.2f seconds' % elapsed)
    ...
    try:
        dataEvol = pickle.load(open(dataset_path + "/data/nonadaptive/tmp/dataEvol_prev"+str(prevTimeslots)+timeTitle+".pck", 'rb'))
    except:        
        print('Please check the parameters and specifically that the timeInterval is the one chosen in stage1.')
